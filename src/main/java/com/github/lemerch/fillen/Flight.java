...
import com.github.lemerch.fillen.exception.service.logical.SetOrIgnoreConflictException;
...
public class Flight {
    ...
    public Flight ignore(String... fieldNames) {
        for (String field : fieldNames) {
            this.context.ignoringlist.put(field, false);
        }
        return this;
    }
    ...
    public<T> Flight set(String fieldName, T value) {
        this.context.settinglist.put(fieldName, new Tuple<>(value));
        return this;
    }
    ...
    public<T> T dinner(Class<T> type) throws BadLootException {
        for (String s : context.settinglist.keySet()) {
            if (context.ignoringlist.containsKey(context.settinglist.get(s).getVal())) {
